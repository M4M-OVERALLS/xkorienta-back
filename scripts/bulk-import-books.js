...
const THEME_RULES = [
  ...
  },
  {
    ...
    descPrefix: 'Réflexion sur le décolonialisme idéologique et le néo-colonialisme.',
  },
  {
...
async function main() {
  ...
  if (!dbUrl) {
    console.error('DATABASE_URL not found. Run with: node -r dotenv/config scripts/bulk-import-books.js');
    process.exit(1);
  }
  ...
  const Book = mongoose.models.Book || mongoose.model('Book', new mongoose.Schema({}, { strict: false }));
  ...
  if (!teacher) {
    ...
    const teachers = await User.find({ role: { $in: ['TEACHER', 'DG_M4M'] } }, { email: 1, name: 1 }).limit(10).lean();
    teachers.forEach(t => console.log(`    - ${t.email} (${t.name})`));
    process.exit(1);
  }
  console.log(`✓ Teacher found: ${teacher.name || teacher.email} (${teacher._id})`);
  ...
  const docxFiles = allFiles.filter(f => path.extname(f).toLowerCase() === '.docx');
  console.log(`   ${pdfFiles.length} PDF files (importable)`);
  console.log(`   ${docxFiles.length} DOCX files (skipped — platform accepts PDF/EPUB only)`);

  // Skip administrative files
  const candidates = pdfFiles.filter(f => !shouldSkip(f));
  const skipped = pdfFiles.filter(f => shouldSkip(f));
  if (skipped.length > 0) {
    console.log(`   ${skipped.length} PDF files skipped (admin/CV/duplicates):`);
  ...
  for (const filename of candidates) {
    ...
    const key = `${stat.size}`;
    if (sizeMap.has(key)) {
      console.log(`   ⚠ Duplicate skipped: ${filename} (same size as ${sizeMap.get(key)})`);
    } else {
  ...
  for (const filename of unique) {
    ...
    const existing = await Book.findOne({
      title: { $regex: new RegExp(`^${title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&').slice(0, 40)}`, 'i') },
  ...
  console.log(`Prix : ${DEFAULT_PRICE === 0 ? 'GRATUIT' : DEFAULT_PRICE + ' ' + DEFAULT_CURRENCY}`);
