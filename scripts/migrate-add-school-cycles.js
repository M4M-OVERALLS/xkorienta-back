...
async function deduceHigherEdCycles(school) {
    ...
    try {
        ...
        console.error('Erreur lors de la déduction des cycles HIGHER_ED:', error.message)
        return [Cycle.SUPERIEUR]
    }
}
    ...
    for (const school of schools) {
        try {
            ...
            if (cycles.length > 0) {
                ...
                console.log(`✅ ${school.name} (${school.type}) → ${cycles.join(', ')}`)
                updated++
            } else {
                console.log(`⏭️ ${school.name} (${school.type}) → Pas de cycles applicables`)
            }
        } catch (error) {
...
async function verifyMigration() {
    ...
    if (schoolsWithoutCycles === 0) {
        ...
        console.log(`   ℹ️ ${schoolsWithoutCycles} écoles sans cycles (normal pour TRAINING_CENTER et OTHER)`)
    }
    ...
    const stats = await School.aggregate([
        { $group: { _id: '$type', count: { $sum: 1 }, cycles: { $addToSet: '$cycles' } } },
        { $sort: { count: -1 } }
    ])
