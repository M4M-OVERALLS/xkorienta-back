...
describe('NotchPayStrategy', () => {
    ...
    describe('initiatePayment', () => {
        it('should return paymentUrl and reference on success', async () => {
            ...
            const result = await strategy.initiatePayment({
                ...
                callbackUrl: 'https://app.quizlock.com/callback',
            })
        ...
        it('should throw when API returns an error', async () => {
            ...
            await expect(strategy.initiatePayment({
                amount: -1,
                currency: 'XAF',
                reference: 'REF',
                email: 'test@test.com',
        ...
        it('should throw when NOTCHPAY_PUBLIC_KEY is not set', async () => {
            ...
            await expect(strategy.initiatePayment({
                amount: 1000, currency: 'XAF', reference: 'REF',
                email: 'test@test.com', description: 'Test', callbackUrl: 'https://cb.com',
    ...
    describe('handleWebhook', () => {
        it('should parse valid webhook with correct signature', async () => {
            ...
            const signature = crypto.createHmac('sha256', 'test_hash_secret').update(raw).digest('hex')
        ...
        it('should throw on invalid signature', async () => {
            const payload = JSON.stringify({ transaction: { reference: 'REF', status: 'complete', amount: 100, currency: 'XAF' } })

            await expect(strategy.handleWebhook(payload, 'invalid-sig'))
                .rejects.toThrow('Invalid NotchPay webhook signature')
        })

        it('should throw when payload has no transaction', async () => {
            const payload = JSON.stringify({ event: 'test' })
            const signature = crypto.createHmac('sha256', 'test_hash_secret').update(payload).digest('hex')

            await expect(strategy.handleWebhook(payload, signature))
                .rejects.toThrow('missing transaction object')
        })
    })
