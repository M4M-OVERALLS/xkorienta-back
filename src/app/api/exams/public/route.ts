...
export async function GET(req: Request) {
    try {
        ...
        const formattedExams = exams.map((exam: any) => ({
            ...
            subject: exam.subject ? { id: exam.subject._id, name: exam.subject.name } : null,
            level: exam.targetLevels?.[0] ? { id: exam.targetLevels[0]._id, name: exam.targetLevels[0].name } : null,
            creatorName: exam.createdById?.name || 'Instructeur Quizlock',
