  ...
  try {
    ...
    switch (type) {
      case "strengths":
        ...
        result =
          await AnalyticsEngine.identifyStrengthsAndWeaknesses(studentId);
        break;
      ...
      case "compare-classes":
        ...
        result = await AnalyticsEngine.compareClasses(
          schoolId,
          subjectId || undefined,
        );
        break;
      ...
      case "compare-schools":
        ...
        if (schoolIds.length === 0) {
          return NextResponse.json(
            { message: "schoolIds required (comma-separated)" },
            { status: 400 },
          );
        }
      ...
      case "subject":
        ...
        result = await AnalyticsEngine.analyzeSubjectPerformance(
          classId,
          subjectId,
        );
        break;
      ...
      case "progression":
        ...
        const endDate = searchParams.get("endDate")
          ...
          : new Date();
        result = await AnalyticsEngine.generateProgressionReport(
          studentId,
          startDate,
          endDate,
        );
      ...
      case "cohort":
        const cohortStudentIds =
          searchParams.get("studentIds")?.split(",") || [];
        const cohortName = searchParams.get("cohortName") || "Cohorte";
        if (cohortStudentIds.length === 0) {
          return NextResponse.json(
            { message: "studentIds required (comma-separated)" },
            { status: 400 },
          );
        }
        result = await AnalyticsEngine.analyzeCohort(
          cohortStudentIds,
          cohortName,
        );
        break;
      ...
      case "correlations":
        const correlationStudentIds =
          searchParams.get("studentIds")?.split(",") || [];
        if (correlationStudentIds.length < 5) {
          return NextResponse.json(
            {
              message:
                "At least 5 studentIds required for correlation analysis",
            },
            { status: 400 },
          );
        }
      ...
      default:
        return NextResponse.json(
          {
            message:
              "type parameter required. Valid values: strengths, compare-classes, compare-schools, subject, progression, cohort, correlations",
          },
          { status: 400 },
        );
    }
