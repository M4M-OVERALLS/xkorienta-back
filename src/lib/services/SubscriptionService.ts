...
/**
 * Extracts a string ID from a Mongoose ref field that may be populated (full document)
 * or a raw ObjectId. Prevents `.toString()` returning `[object Object]` on populated refs.
 */
function resolveId(ref: unknown): string {
  if (
    typeof ref === "object" &&
    ref !== null &&
    "_id" in (ref as Record<string, unknown>)
  ) {
...
export class SubscriptionService {
  ...
  static async subscribe(params: SubscribeParams): Promise<SubscriptionResult> {
    ...
    const existingActive = await subscriptionRepository.findActiveByUser(
      params.userId,
    );
    if (existingActive) {
      throw new Error(
    ...
    if (!price) {
      throw new Error(
        `No price found for ${params.currency} ${params.interval}`,
      );
    }
    ...
    if (!resolvedEmail) {
      throw new Error(
        "Aucun email associé à ce compte. Veuillez ajouter un email dans votre profil pour procéder au paiement.",
      );
    }

    // Initiate payment — skipDuplicateCheck because subscriptions are renewable
  ...
  static async activateSubscription(
    ...
    const transaction =
      await transactionRepository.findByReference(transactionReference);
    if (!transaction) {
    ...
    const existingForPayment =
      await subscriptionRepository.findActiveByLastTransactionId(
    ...
    // resolveId handles populated refs (objects with _id) and raw ObjectIds
    const txUserId = resolveId(transaction.userId);
    const txProductId = resolveId(transaction.productId);

    const plan = await planRepository.findById(txProductId);
    if (!plan) {
      throw new Error("Plan not found");
    }

    const interval =
      (transaction.metadata?.interval as SubscriptionInterval) ??
      SubscriptionInterval.MONTHLY;
    const { start, end } = this.calculatePeriodDates(interval);

    // Check for existing active subscription and expire it
    const existing = await subscriptionRepository.findActiveByUser(txUserId);
    if (existing) {
    ...
    const subscription = await subscriptionRepository.create({
      userId: new mongoose.Types.ObjectId(txUserId),
      planId: new mongoose.Types.ObjectId(txProductId),
      status: SubscriptionPlanStatus.ACTIVE,
      interval,
      currentPeriodStart: start,
      currentPeriodEnd: end,
  ...
  static async renewSubscription(
    ...
    const plan = await planRepository.findById(resolveId(subscription.planId));
    if (!plan) {
      throw new Error("Plan not found");
    }
    if (!plan.isActive) {
    ...
    const renewalPrice =
      plan.prices.find(
        (p) =>
          p.currency === subscription.currency &&
          p.interval === subscription.interval,
      ) ?? plan.prices.find((p) => p.interval === subscription.interval);
    if (!renewalPrice) {
      throw new Error(
        `No price found for ${subscription.currency} ${subscription.interval}`,
      );
    }

    // Initiate payment for renewal — skipDuplicateCheck because subscriptions are renewable
    const paymentResult = await paymentSDK.payments.initiatePayment({
      userId,
      userEmail,
      type: TransactionType.SUBSCRIPTION,
  ...
  static async getUserSubscription(userId: string): Promise<{
    ...
    return {
      ...
      plan,
      isActive:
        subscription.status === SubscriptionPlanStatus.ACTIVE &&
        daysRemaining > 0,
      daysRemaining,
    };
  }
  ...
  static async sendRenewalReminders(daysBeforeExpiry = 3): Promise<number> {
    const subscriptions =
      await subscriptionRepository.findNeedingRenewalReminder(daysBeforeExpiry);
    let sentCount = 0;

    for (const sub of subscriptions) {
      try {
        const plan = await planRepository.findById(resolveId(sub.planId));
        if (!plan) continue;
