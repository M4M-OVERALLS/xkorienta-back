...
export class RequestService {
  ...
  static async getRequests(
    ...
    if (role === UserRole.STUDENT) {
      ...
      const [directRequests, schoolGeneralRequests, externalRequests] =
        await Promise.all([
  ...
  static async createRequest(
    ...
    if (data.relatedConceptIds && data.relatedConceptIds.length > 0) {
      requestData.relatedConcepts = data.relatedConceptIds.map(
    ...
    // Notifications for assistance request creation
    // - student requester confirmation
    // - concerned teacher(s): direct teacher or class teachers
    const teacherRecipientIds = new Set<string>();
    if (data.teacherId) {
      teacherRecipientIds.add(data.teacherId);
    } else if ((data.teacherType || TeacherType.SCHOOL) === TeacherType.SCHOOL) {
      const studentClasses = await Class.find({
        students: new mongoose.Types.ObjectId(studentId),
        isActive: true,
      })
        .select("mainTeacher teachers.teacher")
        .lean();

      for (const cls of studentClasses) {
        if (cls.mainTeacher) {
          teacherRecipientIds.add(cls.mainTeacher.toString());
        }
        for (const entry of cls.teachers || []) {
          if (entry?.teacher) {
            teacherRecipientIds.add(entry.teacher.toString());
          }
        }
      }
    }

    // Never notify requester as teacher recipient
    teacherRecipientIds.delete(studentId);
  ...
  static async claimExternalRequest(requestId: string, teacherId: string) {
    ...
    if (pusher) {
      pusher.trigger(
        getRequestsChannel(requestData.studentId.toString()),
        "request-claimed",
        {
  ...
  static async updateRequest(
    ...
    if (updateData.status === RequestStatus.ACCEPTED && isTeacher) {
      ...
      if (updateData.scheduledDuration)
        request.scheduledDuration = updateData.scheduledDuration;
    ...
    const pusher = getPusherServer();
    if (pusher) {
      const targetUserId = isTeacher
        ? request.studentId._id.toString()
        : request.teacherId?._id?.toString();
      if (targetUserId) {
