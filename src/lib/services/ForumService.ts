...
export class ForumService {
  ...
  async getForums(
    ...
    const query: any = { status: ForumStatus.ACTIVE };
  ...
  async getForumById(forumId: string, userId: string) {
    ...
    const isMember =
      (forum as any).members.some((m: any) => m._id.toString() === userId) ||
  ...
  async createForum(
    ...
    const type = data.type || ForumType.CLASS;

    // Enforce payload consistency by forum type.
    if (type === ForumType.CLASS && !data.classId) {
      throw new Error("classId requis pour un forum de type CLASS");
    }
    if (type === ForumType.SUBJECT && !data.subjectId) {
      throw new Error("subjectId requis pour un forum de type SUBJECT");
    }
    if (type === ForumType.GENERAL && (data.classId || data.subjectId)) {
      throw new Error(
        "Un forum GENERAL ne doit pas avoir de classId/subjectId",
      );
    }
    ...
    if (data.classId) {
      ...
      if (classData && (classData as any).students) {
        const studentIds = (classData as any).students.map(
          (s: any) => new mongoose.Types.ObjectId(s.toString()),
        );
    ...
    const forum = await this.forumRepository.create({
      ...
      type,
      relatedClass: data.classId
        ? new mongoose.Types.ObjectId(data.classId)
        : undefined,
      relatedSubject: data.subjectId
    ...
    await this.forumRepository.populateRelations(forum, [
      "createdBy",
      "relatedClass",
    ]);
    ...
    // Notifications: class members + teacher creator
    const creatorId = userId;
    const recipientIds = uniqueMembers.map((id) => id.toString());
    const memberNotifications = recipientIds
      .filter((id) => id !== creatorId)
      .map((id) => ({
        userId: new mongoose.Types.ObjectId(id),
        type: "info" as const,
        title: "Nouveau forum disponible",
        message: `Un nouveau forum "${data.name}" a été créé.`,
        read: false,
        data: {
          forumId: forum._id,
          forumName: data.name,
    ...
    await Notification.create({
      ...
      type: "success",
      title: "Forum créé",
      message: `Votre forum "${data.name}" a été créé et notifié aux membres concernés.`,
      read: false,
      data: {
        forumId: forum._id,
        forumName: data.name,
  ...
  async updateForum(
    ...
    if (data.allowStudentPosts !== undefined)
      forum.allowStudentPosts = data.allowStudentPosts;
