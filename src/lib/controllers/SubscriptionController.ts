...
export class SubscriptionController {
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Missing required fields: planCode, interval, currency, callbackUrl",
        },
        { status: 400 },
      );
    }
  ...
  static async cancelMine(session: AuthSession) {
    ...
    return NextResponse.json({
      ...
      data: subscription,
      message:
        "Subscription cancelled successfully. You will retain access until the end of your current period.",
    });
  }
  ...
  static async getHistory(session: AuthSession) {
    const subscriptions = await SubscriptionService.getUserSubscriptionHistory(
      session.user.id,
    );
